					...
					if ( in_array( $prop['listingID'], $listings, true ) && ! isset( $idx_featured_listing_wp_options[ $prop['listingID'] ]['post_id'] ) ) {
						...
						if ( class_exists( 'Equity_Idx_Api' ) ) {
							...
							if ( $equity_properties == false ) {
								// The Equity API returned no data for the property
								// This is usually caused by the WordPress domain not matching the approved domain in the IDX account.
								// Only some data has been imported.
								$equity_properties = $properties[$key];
								delete_transient( 'equity_listing_' . $prop['listingID'] );
							}
						}

						if ( ! isset( $properties[$key]['address'] ) || $properties[$key]['address'] == '' ) {
						...
						if ( class_exists( 'Equity_Idx_Api' ) ) {
							$item['equity_properties'] = $equity_properties;
						}
	...
	public static function wp_listings_idx_insert_post_meta($id, $idx_featured_listing_data, $update = false, $update_image = true, $sold = false) {
		...
		if($update == false || $update_image == true) {
			...
			if ( substr( $image_url, 0, 2 ) === "//" ) {
				$image_data = file_get_contents("https:" . $image_url);
				//try http if fail file_get_contents fails with https
				if ( $image_data === FALSE ) {
					$image_data = file_get_contents("http:" . $image_url);
				}
			} else {
...
function sync_listing_options() {
	...
	$idx_options = get_option( 'wp_listings_idx_featured_listing_wp_options' );

	if ( is_array( $listing_posts ) && is_array( $idx_options ) ) {
		...
		foreach ( $listing_posts as $key => $value) {
			...
			if ( array_key_exists( '_listing_mls', $listing_post_meta ) && ! empty( $listing_post_meta['_listing_mls'] ) ) {
				// If key does not exist in $idx_options -> create it and add all values.
				if ( ! array_key_exists( $listing_post_meta['_listing_mls'][0], $idx_options ) ) {
					$idx_options[ $listing_post_meta['_listing_mls'][0] ] = [
					...
					// If key does exist in $idx_options -> just add missing values.
					$listing_options = $idx_options[ $listing_post_meta['_listing_mls'][0] ];
					...
					if ( ! isset( $listing_options['listingID'] ) || empty( $listing_options['listingID'] ) ) {
						$idx_options[ $listing_post_meta['_listing_mls'][0] ]['listingID'] = $listing_post_meta['_listing_mls'][0];
					}
					// updated.
					if ( ! isset( $listing_options['updated'] ) || empty( $listing_options['updated'] ) ) {
						$idx_options[ $listing_post_meta['_listing_mls'][0] ]['updated'] = date("m/d/Y h:i:sa");
					}
					// status.
					if ( ! isset( $listing_options['status'] ) || empty( $listing_options['status'] ) ) {
						$idx_options[ $listing_post_meta['_listing_mls'][0] ]['status'] = $value->post_status;
					}
					// post_id.
					if ( ! isset( $listing_options['post_id'] ) || empty( $listing_options['post_id'] ) ) {
						$idx_options[ $listing_post_meta['_listing_mls'][0] ]['post_id'] = $value->ID;
					}
				}
				update_option( 'wp_listings_idx_featured_listing_wp_options', $idx_options );
			}
		}
	}
}
				...
				<div class="grid-item post">
					<label for="%s" class="idx-listing">
						<li class="%s">
							...
							<input type="checkbox" id="%s" class="checkbox" name="wp_listings_idx_featured_listing_options[]" value="%s" %s />%s
...
class WPLBackgroundListings extends WP_Background_Process {
	...
	protected function task( $data ) {
		...
		$key               = $data['key'];
		$opts              = $data['opts'];
		if ( array_key_exists( 'equity_properties', $data ) ) {
			$equity_properties = $data['equity_properties'];
		}
		...
		if ( is_wp_error( $add_post ) ) {
			...
			if ( class_exists( 'Equity_Idx_Api' ) ) {
				WPL_Idx_Listing::wp_listings_idx_insert_post_meta($add_post, $equity_properties);
			} else {
